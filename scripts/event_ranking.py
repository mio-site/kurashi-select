...
def parse_args():
    p = argparse.ArgumentParser(description="イベントモード：ジャンル指定ランキング + 記事JSON更新")
    p.add_argument("--genres", help="カンマ区切りのgenreId。未指定で全トップ", default="")
...
def main():
    ...
    out_name = f"ranking_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')}_event.jsonl"
    out_path = Path(out_name)

    genres = [g.strip() for g in (args.genres or "").split(",") if g.strip()]
    if not genres:
        # 全トップ（root children）
        gtree = client.fetch_genre(0)
        genres = [c.get("child", {}).get("genreId") for c in gtree.get("children", [])]

    with out_path.open("a", encoding="utf-8") as fw:
        for gid in genres:
            try:
                gdata = client.fetch_genre(gid)
                gname = gdata.get("current", {}).get("genreName") or str(gid)
            except Exception:
                gname = str(gid)
            try:
    ...
    changed = build_article_json(out_path, args.genre, args.top_n, Path("site_data/series001/001-001.json"))
    print(f"event ranking file: {out_path}")
